...
class _ProjectFileExplorer extends React.Component {
    ...
    renderFile(path) {
        ...
        return (<FileEntry selected={this.isSelected(full)} language={this.getLanguage(full)} blacklist={blacklist}
                           label={path} key={path} select={() => this.props.toggle(full)}
                           setLanguage={(lang) => this.props.setLang(full, lang)}/>);
    }
    ...
    renderProjectPath() {
        ...
        return (<React.Fragment>
            <BreadcrumbItem tag="a" href="#"
                            onClick={() => this.props.browse('/')}>{this.props.projectName}</BreadcrumbItem>
            {list.map((fragment, index) => {
                let path = '/' + list.slice(0, index + 1).join('/') + '/';
                return (
                    <BreadcrumbItem tag="a" href="#" title={path} key={path} onClick={() => this.props.browse(path)}>
                        {fragment}
                    </BreadcrumbItem>);
            })}
        </React.Fragment>);
