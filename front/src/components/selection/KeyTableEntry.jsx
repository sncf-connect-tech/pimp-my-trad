...
import { UncontrolledDropdown, DropdownItem, DropdownMenu, DropdownToggle, ListGroupItem, Row, Col, Input, Button } from 'reactstrap';
    ...
    renderFocused() {
        return (<div>
            ...
            <div className="alert alert-danger mb-1">
                ...
                <Input type="textarea" onChange={e => this.updateText(e)} onBlur={e => this.modifyKey()} value={this.state.text}/>
                <div className="mt-1">
                    <UncontrolledDropdown className="d-inline-block">
                        <DropdownToggle caret>{getPrettyLanguage(this.state.language)}</DropdownToggle>
                        <DropdownMenu>
                            {this.props.languages.map(lang => <DropdownItem key={lang} disabled={lang === this.state.language} onClick={() => this.switchLang(lang)}>{getPrettyLanguage(lang)}</DropdownItem>)}
                        </DropdownMenu>
        ...
        return (<ListGroupItem tag="li" action={!this.state.focused} onClick={() => this.state.focused ? null : this.focus()}>
            ...
            <Col xs={7}><span className="">{this.defaultText()} </span></Col>}
                    <Col xs={2} className="text-right">
                        <Button className="text-uppercase" size="sm" color={this.getColor()} onClick={e => this.filterState(e)}>
