        ...
        return (<Modal fade={true} isOpen={this.props.show} toggle={() => this.props.hide()}>
                ...
                <ModalBody>
                    <FormGroup>
                        ...
                        <Input type="text" placeholder="ex: myapp.new.key" value={this.state.keyId} valid={this.state.valid} invalid={!this.state.valid} onChange={this.updateStateValue('keyId')}/>
                        <FormFeedback>Cette clé existe déjà!</FormFeedback>
                    </FormGroup>
                    <FormGroup>
                        <Label>Texte français</Label>
                        <Input type="text" placeholder="ex: 'Nouvelle clé'" value={this.state.defaultText} onChange={this.updateStateValue('defaultText')}/>
                    </FormGroup>
                    <FormGroup>
            ...
            .map(keyset => <option key={keyset.id} value={keyset.id}>{keyset.name}</option>)}
                ...
                <ModalFooter>
                    <LoadingButton color="primary" onClick={() => this.addKey()} disabled={!this.state.valid}>Ajouter</LoadingButton>{' '}
